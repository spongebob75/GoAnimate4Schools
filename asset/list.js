	...
	switch (data.type) {
		...
		case "sound": {
			var files = asset.list(data.movieId, "music");
			 xmlString = `${header}<ugc more="0">${files
				 .map((v) => `<sound subtype="bgmusic" id="${v.id}" name="${v.name}" enable="Y" duration="336804" downloadtype="progressive"/>`)
				 .join("")}</ugc>`;
			break;
		}
		case "movie": {
			...
			xmlString = `${header}<ugc more="0">${files
				.map((v) => `<movie id="${v.id}" path="/_SAVED/${v.id}" numScene="1" title="${v.name}" thumbnail_url="/movie_thumbs/${v.id}.png"><tags></tags></movie>`)
				.join("")}</ugc>`;
			break;
		}
		case "prop":
